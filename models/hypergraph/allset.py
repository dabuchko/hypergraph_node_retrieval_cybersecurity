...
class PMA(MessagePassing):
    ...
    def __init__(self, in_channels, hid_dim,
        ...
        super(PMA, self).__init__('add', node_dim=0, **kwargs)
...
class HalfNLHconv(MessagePassing):
    def __init__(self,
        ...
        super(HalfNLHconv, self).__init__(aggr)
...
class SetGNN(nn.Module):
    ...
    def __init__(self, in_channels: int, hidden_channels: int, num_layers: int, out_channels: int,
                 dropout, classifier_hidden, classifier_num_layers, aggregate, normalization,
                 deepset_input_norm, GPR, heads, PMA):
...
#         If there's hyperedge features, concat with Vpart decoder output features [V_feat||E_feat]
        ...
        if self.All_num_layers == 0:
            ...
            self.V2EConvs.append(HalfNLHconv(in_dim=in_channels,
            ...
            self.E2VConvs.append(HalfNLHconv(in_dim=hidden_channels,
                                             hid_dim=hidden_channels,
                                             out_dim=hidden_channels,
                                             num_layers=num_layers,
                                             dropout=self.dropout,
            ...
            for _ in range(self.All_num_layers-1):
                self.V2EConvs.append(HalfNLHconv(in_dim=hidden_channels,
                                                 hid_dim=hidden_channels,
                                                 out_dim=hidden_channels,
                                                 num_layers=num_layers,
                                                 dropout=self.dropout,
