...
class Hypergraph:
    ...
    1. `hyperedge_index` -- torch.Tensor matrix with two rows, where first row
    ...
    def __init__(self, hyperedge_index: torch.Tensor, y: torch.Tensor=None,
        ...
        :param self:
        :param hyperedge_index: torch.Tensor matrix with two rows, where first row
        represents node id and second row represents hyperedge id. It encodes
        belongingness of specific node to specific hyperedge.
        ...
        if val_mask!=None and (val_mask.dtype!=torch.bool or len(val_mask.shape)!=1):
    ...
    def incidence_matrix(self) -> torch.Tensor:
        ...
        for i in range(self.hyperedge_index.shape[1]):
            node = int(self.hyperedge_index[0, i].item())
            hyperedge = int(self.hyperedge_index[1, i].item())
